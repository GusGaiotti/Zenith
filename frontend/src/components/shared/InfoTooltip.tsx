...
export function InfoTooltip({ text, align = "right" }: InfoTooltipProps) {
  return (
    <span className="group relative z-20 inline-flex items-center">
      ...
      <span
        className={`pointer-events-none absolute bottom-full z-50 mb-3 w-64 max-w-[calc(100vw-2rem)] translate-y-1 rounded-xl border border-[var(--surface-edge)] bg-[var(--tooltip-bg)] px-3 py-2 text-left text-xs leading-5 text-white break-words opacity-0 shadow-[0_18px_50px_rgba(0,0,0,0.24)] transition-all duration-150 group-hover:translate-y-0 group-hover:opacity-100 group-focus-within:translate-y-0 group-focus-within:opacity-100 ${align === "left" ? "left-0" : "right-0"}`}
        role="tooltip"
      >
        {text}
      </span>
